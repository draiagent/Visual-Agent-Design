...
### 1.1 核心原則｜High Intelligence for Discovery, Low Cost for Execution

> 第一次用高階 AI「學會」，之後讓低成本 Agent「重複做好」。
> 高智能負責探索，低成本負責規模化執行。

這條原則決定了 VAD 在路由與資產化上的立場，並對應到三種功能：

- **Visual Requirement Interface**：用圖像降低人與 Agent 的溝通成本，讓「探索」階段的高階模型能更快、更準確地理解任務。
- **Reusable Task Specification**：把已經探索成功的任務固化成 VAC / Skill，成為可重複執行的規格，而不必每次重新用高階模型摸索。
- **Cost-Aware Intelligence Routing**：TRC-3D 判斷任務已知程度後，已知、重複的任務交由低成本模型或固定 Workflow 執行；只有未知、單次、需要複雜推理的任務才升級高階模型（見 §2.1）。

因此 VAD 不追求「企業全面使用最強 AI 模型」，而是追求「用最強模型換取正確性一次，再用最省成本的模型換取規模化執行」——這是 §3 生命週期中 LEARN 階段把案例回存為標準卡的根本理由。
